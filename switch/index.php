<?php
	...
	try
	{
	$sql2 = 'SELECT username, work_info.userid, work_info.timeid, timedate, starttime, endtime FROM work_info INNER JOIN time_info on work_info.timeid = time_info.timeid  INNER JOIN user_info WHERE work_info.userid != :userid AND user_info.userid = work_info.userid AND work_info.timeid not in ( SELECT work_info.timeid FROM  work_info WHERE work_info.userid = :userid )';
	$s2 = $pdo->prepare($sql2);
